        ...
        try {
            ...
            if (response && response.displayName && response['@odata.type']) {
                ...
                if (response["@odata.type"] === '#microsoft.graph.group') {
                    ...
                    return new GuidResolverResponse(response.id, response.displayName, 'Microsoft Entra ID Group', response, new Date());
                }
                else if (response["@odata.type"] === '#microsoft.graph.user') {
                    abortController.abort();
                    return new GuidResolverResponse(response.id, response.displayName, 'Microsoft Entra ID User', response, new Date());
                }
                else if (response["@odata.type"] === '#microsoft.graph.servicePrincipal') {
                    abortController.abort();
                    return new GuidResolverResponse(response.id, response.displayName, 'Microsoft Entra ID ServicePrincipal', response, new Date());
                }
                else if (response["@odata.type"] === '#microsoft.graph.application') {
                    abortController.abort();
                    return new GuidResolverResponse(response.id, response.displayName, 'Microsoft Entra ID AppRegistration', response, new Date());
                }
                else if (response["@odata.type"] === '#microsoft.graph.tokenLifetimePolicy') {
                    abortController.abort();
                    return new GuidResolverResponse(response.id, response.displayName, 'Microsoft Entra ID TokenLifetimePolicy', response, new Date());
                }
                else if (response["@odata.type"] === '#microsoft.graph.directoryRole') {
                    abortController.abort();
                    return new GuidResolverResponse(response.id, response.displayName, 'Microsoft Entra ID DirectoryRole', response, new Date());
                }
                else {
